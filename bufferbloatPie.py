...
from mininet.log import debug, lg, info, setLogLevel
...
class BBTopo(Topo):
    ...
    def __init__(self, n=2):
        ...
        self.addLink(s0, h2, 2, 2, bw=args.bw_net, delay=args.delay, loss=0.05)
        return
...
# Queue monitoring
def start_qmon(iface, interval_sec=0.1, outfile=args.dir+"/q.txt"):
...
def start_ping(net, outfile=args.dir+"/ping.txt"):
    ...
    with open(outfile, "w+") as f :
        h1.popen("ping %s -i 0.1" % h2.IP(), shell=True, stdout=f)
...
def bufferbloat():
    ...
    h1, h2 = net.getNodeByName('h1', 'h2')
    h1.popen("curl -o /dev/null -s -w %{time_total} %s", h2.IP(), shell=True)
